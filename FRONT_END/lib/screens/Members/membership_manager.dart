...
class MemberManager extends ChangeNotifier {
  ...
  Future<void> fetchMembers(String groupId, {String? myUserId}) async {
    ...
    try {
      ...
      // If group not found, treat as empty list and cache
      if (err.contains("requested resource wasn't found") ||
          err.contains('404')) {
        debugPrint('⚠️ Group $groupId not found; caching empty member list.');
        _cache(groupId, <Map<String, dynamic>>[], myUserId: myUserId);
      } else {
