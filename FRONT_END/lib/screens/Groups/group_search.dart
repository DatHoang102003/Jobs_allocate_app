...
Future<UserGroupStatus> checkUserGroupStatus({
  ...
  if (joinManager.isPending(groupId)) {
    return UserGroupStatus.pending;
  }
...
class _GroupSearchState extends State<GroupSearch> {
  ...
  Future<void> _handleSearch(String keyword) async {
    // allow UI to update before search
    await Future.delayed(Duration.zero);
    // if widget disposed, abort
    if (!mounted) return;
    ...
    try {
      ...
      final results = await groupProvider.searchGroups(query);
      if (!mounted) return;
      setState(() => _searchResults = results);

      // refresh join requests but no UI change needed here
      await context.read<JoinManager>().fetchJoinRequests();
    } catch (e) {
      // avoid calling context if disposed
      if (mounted) debugPrint("Search failed: $e");
    }
  }
                  ...
                  onPressed: (userStatus == UserGroupStatus.admin ||
                      ...
                      : () async {
                          try {
                            ...
                            if (mounted) debugPrint('Action failed: $e');
                          }
                        },
